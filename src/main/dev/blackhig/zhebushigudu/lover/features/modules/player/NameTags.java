...
public class NameTags extends Module
{
    ...
    public NameTags() {
        super("NameTags", "Better Nametags", Module.Category.PLAYER, false, false, false);
    ...
    public void onRender3D(final Render3DEvent event) {
        if (!fullNullCheck()) {
            for (final EntityPlayer player : NameTags.mc.world.playerEntities) {
                if (player.getDistance((Entity)NameTags.mc.player) <= 30.0 && player != null && !player.equals((Object)NameTags.mc.player) && player.isEntityAlive()) {
    ...
    private void renderNameTag(final EntityPlayer player, final double x, final double y, final double z, final float delta) {
        ...
        if (NameTags.$assertionsDisabled || camera != null) {
            ...
            final double scale = (distance <= 6.0) ? 0.0245 : ((0.0018 + 4.5 * (1.0 * distance)) / 1000.0);
            ...
            GlStateManager.pushMatrix();
            int xOffset = -8;
            final Iterator it = player.inventory.armorInventory.iterator();
            while (it.hasNext()) {
                if (it.next() == null) {
                    continue;
                }
            ...
            this.renderItemStack(player.getHeldItemOffhand().copy(), xOffset2, -26);
            int xOffset3 = xOffset2 + 16;
            for (final ItemStack stack2 : player.inventory.armorInventory) {
                if (stack2 == null) {
                    continue;
                }
                this.renderItemStack(stack2.copy(), xOffset3, -26);
                xOffset3 += 16;
            }
            this.renderItemStack(renderMainHand, xOffset3, -26);
