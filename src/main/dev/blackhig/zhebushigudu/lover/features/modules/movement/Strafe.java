    ...
    public void onStrafe(final MoveEvent event) {
        if (!Feature.fullNullCheck() && !Util.mc.player.isInWater() && !Util.mc.player.isInLava()) {
            ...
            switch (this.stage) {
                ...
                default: {
                    if ((Util.mc.world.getCollisionBoxes((Entity)Util.mc.player, Util.mc.player.getEntityBoundingBox().offset(0.0, Util.mc.player.motionY, 0.0)).size() > 0 || Util.mc.player.collidedVertically) && this.stage > 0) {
                ...
                case 2: {
                    ...
                    if (Util.mc.player.onGround && Util.mc.gameSettings.keyBindJump.isKeyDown()) {
                        if (Util.mc.player.isPotionActive(MobEffects.JUMP_BOOST)) {
                            motionY = 0.40123128 + (Util.mc.player.getActivePotionEffect(MobEffects.JUMP_BOOST).getAmplifier() + 1) * 0.1f;
                        }
