...
public class BedMiner extends Module
{
    ...
    private EntityPlayer getTarget(final double range) {
        ...
        for (final EntityPlayer player : BedMiner.mc.world.playerEntities) {
            if (!EntityUtil.isntValid((Entity)player, range) && !lover.friendManager.isFriend(player.getName()) && BedMiner.mc.player.posY - player.posY < 5.0 && lover.speedManager.getPlayerSpeed(player) <= this.targetMaxSpeed.getValue()) {
