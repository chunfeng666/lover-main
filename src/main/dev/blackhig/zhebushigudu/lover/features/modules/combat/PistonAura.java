...
public class PistonAura extends Module
{
    ...
    public PistonAura() {
        ...
        this.range = (Setting<Double>)this.register(new Setting("Range", (T)5.2, (T)0, (T)15));
        this.delay1 = (Setting<Integer>)this.register(new Setting("ChangeDelay", (T)5, (T)0, (T)20));
        this.delay2 = (Setting<Integer>)this.register(new Setting("PlaceDelay", (T)2, (T)(-1), (T)100));
        this.min = (Setting<Integer>)this.register(new Setting("MinDamage", (T)21, (T)0, (T)100));
        this.thread = (Setting<Integer>)this.register(new Setting("Thread", (T)1, (T)0, (T)10));
        this.progress = 0;
    }
    ...
    public void onEnable() {
        ...
        this.attack = new CopyOnWriteArrayList<PA>();
        super.onEnable();
    }
    ...
    public void onTick() {
        ...
        if (this.attackable.isEmpty() || PistonAura.mc.player.ticksExisted % Math.max(1, this.delay1.getValue() / Math.max(1, this.thread.getValue())) == 0) {
            this.attackable = new ArrayList<PA>();
            for (int dx = (Object)(-range); dx <= range; ++dx) {
                for (int dy = (Object)(-range); dy <= range; ++dy) {
                    for (int dz = (Object)(-range); dz <= range; ++dz) {
                        final BlockPos pos = new BlockPos((Entity)PistonAura.mc.player).add(dx, dy, dz);
                        if (player.getDistanceSq(pos) <= range * range) {
                            boolean b = false;
                            for (final BlockPos off : PistonAura.pistonoff) {
                                final Block block = PistonAura.mc.world.getBlockState(pos.add((Vec3i)off)).getBlock();
                            ...
                            if (b) {
                                final double damage = getDamage(new Vec3d((Vec3i)pos).add(0.5, 0.0, 0.5), TargetUtils.currentTarget);
                ...
                this.attackable.sort(new Comparator<PA>() {
                    @Override
                    public int compare(final PA a, final PA b) {
                        if (a == null && b == null) {
                            return 0;
                        }
    ...
    public void onmotion(final EventMotion e) {
        ...
        for (final Entity et : PistonAura.mc.world.loadedEntityList) {
            if (et instanceof EntityEnderCrystal) {
                if (!TargetUtils.canAttack(PistonAura.mc.player.getPositionVector().add(0.0, (double)PistonAura.mc.player.getEyeHeight(), 0.0), et.getPositionVector().add(0.0, 1.7, 0.0))) {
                    continue;
                }
                PistonAura.mc.playerController.attackEntity((EntityPlayer)PistonAura.mc.player, et);
    ...
    public EnumFacing getFacing(final BlockPos position) {
        for (final EnumFacing f : EnumFacing.values()) {
            if (f.getAxis() != EnumFacing.Axis.Y) {
                final BlockPos pos = new BlockPos((Vec3i)position).offset(f, 2);
    ...
    public static boolean placeCrystalSilent(final BlockPos pos) {
        ...
        if (((ItemStack)PistonAura.mc.player.inventory.offHandInventory.get(0)).getItem() == Items.END_CRYSTAL) {
            PistonAura.mc.getConnection().sendPacket((Packet)new CPacketPlayerTryUseItemOnBlock(pos.offset(EnumFacing.DOWN), EnumFacing.UP, EnumHand.OFF_HAND, 0.0f, 0.0f, 0.0f));
        }
        else if (InventoryUtil.pickItem(426, false) != -1) {
            PistonAura.mc.getConnection().sendPacket((Packet)new CPacketHeldItemChange(slot));
            PistonAura.mc.getConnection().sendPacket((Packet)new CPacketPlayerTryUseItemOnBlock(pos.offset(EnumFacing.DOWN), EnumFacing.UP, EnumHand.MAIN_HAND, 0.0f, 0.0f, 0.0f));
        }
        return true;
    }
    ...
    public static double getDamage(final Vec3d pos, @Nullable final Entity target) {
        final Entity entity = (Entity)((target == null) ? PistonAura.mc.player : target);
    ...
    public class PA
    {
                    ...
                    if (this.pistonFacing != EnumFacing.DOWN) {
                        ...
                        if (Util.mc.world.isAirBlock(crystallook)) {
                            if (Util.mc.world.isAirBlock(crystalhead)) {
                                if (PistonAura.canAttack(Util.mc.player.getPositionVector().add(0.0, (double)Util.mc.player.getEyeHeight(), 0.0), new Vec3d((Vec3i)crypos).add(0.5, 1.7, 0.5))) {
                                    ...
                                    if (b1 instanceof BlockObsidian || b1 instanceof BlockEmptyDrops) {
                                        if (Util.mc.player.getDistanceSqToCenter(crypos) < 64.0) {
                                            if (Util.mc.world.checkNoEntityCollision(Block.FULL_BLOCK_AABB.offset(crypos))) {
                                                this.crystal = crypos;
                                                for (final BlockPos off : PistonAura.pistonoff) {
                                                    final BlockPos pispos = this.crystal.add((Vec3i)off);
                                                    final BlockPos pistonhead = pispos.offset(this.pistonFacing);
                                                    Label_1198: {
                                                        if (!pispos.equals((Object)crypos)) {
                                                            if (!crystalhead.equals((Object)pispos)) {
                                                                if (!crystallook.equals((Object)pispos)) {
                                                                                ...
                                                                                if (Util.mc.player.getDistanceSqToCenter(pispos) < 64.0) {
                                                                                    ...
                                                                                    if (this.pistonFacing.getAxis() != EnumFacing.Axis.Y || this.pistonFacing == sfac) {
                                                                                        if (sfac.getAxis() != EnumFacing.Axis.Y || this.pistonFacing == sfac) {
                                                                                            if (!Util.mc.world.getEntitiesWithinAABBExcludingEntity((Entity)null, Block.FULL_BLOCK_AABB.offset(pistonhead)).contains(Util.mc.player)) {
                                                                                                this.power = null;
                                                                                                ...
                                                                                                if (Util.mc.world.isBlockPowered(pispos)) {
                                                                                                    ...
                                                                                                    for (final EnumFacing fa : EnumFacing.values()) {
                                                                                                        ...
                                                                                                        if (!pispos.equals((Object)powpos)) {
                                                                                                            if (!pistonhead.equals((Object)powpos)) {
                                                                                                                if (!crypos.equals((Object)powpos)) {
                                                                                                                    if (!crystalhead.equals((Object)powpos)) {
        ...
        public void updatePA(final EventMotion event) {
            ...
            if (PistonAura.this.delay2.getValue() == -1) {
                Util.mc.getConnection().sendPacket((Packet)new CPacketPlayer.Rotation(event.yaw, event.pitch, Util.mc.player.onGround));
            }
            ...
            Util.mc.getConnection().sendPacket((Packet)new CPacketPlayerDigging(CPacketPlayerDigging.Action.STOP_DESTROY_BLOCK, this.piston, EnumFacing.UP));
            if (this.power != null) {
                Util.mc.world.setBlockToAir(this.power);
                Util.mc.getConnection().sendPacket((Packet)new CPacketPlayerDigging(CPacketPlayerDigging.Action.STOP_DESTROY_BLOCK, this.power, EnumFacing.UP));
            }
            ++this.stage;
            Util.mc.getConnection().sendPacket((Packet)new CPacketHeldItemChange(InventoryUtil.getSlot()));
        }
    }
}
