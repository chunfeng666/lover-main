...
public class AutoQueue extends Module
{
    ...
    public void onClientChatReceived(final ClientChatReceivedEvent event) {
        ...
        this.m_questionAndAnswer.entrySet().stream().filter(p -> msg.contains(p.getKey())).findFirst().ifPresent(Answer -> AutoQueue.mc.player.connection.sendPacket((Packet)new CPacketChatMessage((String)Answer.getValue())));
    }
