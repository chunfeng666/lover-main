...
public class HoleSnap extends Module
{
    ...
    public HoleSnap() {
        super("HoleSnap", "IQ", Module.Category.MOVEMENT, true, false, false);
        this.range = (Setting<Integer>)this.register(new Setting("Range", (T)5, (T)1, (T)50));
        this.timer = (Setting<Float>)this.register(new Setting("TimerVal", (T)3.4f, (T)1.0f, (T)4.0f));
        this.timeoutTicks = (Setting<Integer>)this.register(new Setting("TimeOutTicks", (T)60, (T)0, (T)1000));
    }
    ...
    public void onMove(final MoveEvent2 event) {
        ...
        if (this.enabledTicks > n.intValue()) {
            this.disable();
            return;
        }
        if (Module.mc.player.isEntityAlive()) {
            if (!this.isFlying((EntityPlayer)HoleSnap.mc.player)) {
                final EntityPlayerSP entityPlayerSP = Module.mc.player;
                final double currentSpeed = this.getSpeed((Entity)entityPlayerSP);
                ...
                if (blockPos != null) {
                    ...
                    if (!this.isCentered((Entity)HoleSnap.mc.player, blockPos)) {
                        ...
                        final double baseSpeed = this.applySpeedPotionEffects((EntityLivingBase)entityPlayerSP2);
    ...
    private boolean shouldDisable(final double currentSpeed) {
        ...
        if (blockPos != null) {
            ...
            if (HoleUtil.is2HoleB(blockPos)) {
                ...
                if (VectorUtils.INSTANCE.toBlockPos(VectorUtils.INSTANCE.toVec3dCenter((Vec3i)blockPos2)).equals((Object)blockPos)) {
                    return true;
                }
            }
        }
        ...
        final EntityPlayerSP entityPlayerSP = Module.mc.player;
        return SurroundUtils.INSTANCE.checkHole((Entity)entityPlayerSP) != SurroundUtils.HoleType.NONE;
    }
    
    private BlockPos getHole() {
        if (Module.mc.player.ticksExisted % 10 == 0) {
            final EntityPlayerSP entityPlayerSP = Module.mc.player;
            if (!SurroundUtils.INSTANCE.getFlooredPosition((Entity)entityPlayerSP).equals((Object)this.holePos)) {
                final BlockPos blockPos = this.findHole();
                return blockPos;
            }
        }
    ...
    private BlockPos findHole() {
        Pair<Double, BlockPos> closestHole = new Pair<Double, BlockPos>(69.69, BlockPos.ORIGIN);
        final EntityPlayerSP entityPlayerSP = Module.mc.player;
        final BlockPos playerPos = SurroundUtils.INSTANCE.getFlooredPosition((Entity)entityPlayerSP);
        final Integer ceilRange = this.range.getValue();
        final BlockPos blockPos2 = playerPos.add((int)ceilRange, -1, (int)ceilRange);
        ...
        for (final BlockPos posXZ : posList) {
            final EntityPlayerSP entityPlayerSP2 = Module.mc.player;
            final double dist = VectorUtils.INSTANCE.distanceTo((Entity)entityPlayerSP2, (Vec3i)posXZ);
            final Integer n = this.range.getValue();
            if (dist <= ((n == null) ? null : Double.valueOf(n))) {
                if (dist > closestHole.getLeft().doubleValue()) {
                    continue;
                }
                int n2 = 0;
                BlockPos pos;
